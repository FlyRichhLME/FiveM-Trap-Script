...
local function StartDeal(customerPed, playerVeh)
    ...
    while GetGameTimer() < timeout do
        Wait(500)
        if IsPedInVehicle(customerPed, playerVeh, false) then
            break
        end
    end
...
local function SpawnCustomer()
    ...
    if playerVeh == 0 then
        Notify('You need to be inside your vehicle.', 'error')
        ResetTrapState()
        return
    end
    ...
    local spawn = vector3(
        coords.x + math.cos(angle) * Config.SpawnDistance,
        coords.y + math.sin(angle) * Config.SpawnDistance,
        coords.z
    )

    local foundGround, groundZ = GetGroundZFor_3dCoord(spawn.x, spawn.y, spawn.z + 50.0, false)
    if foundGround then
        spawn = vector3(spawn.x, spawn.y, groundZ)
    end
    ...
    if currentMode == 'bulk' then
        Notify('Bulk buyer is walking to your passenger door.', 'success')
    else
    ...
    CreateThread(function()
        ...
        while trapActive and currentCustomer == customerPed and DoesEntityExist(customerPed) do
            ...
            if Config.RequirePassengerSeatOpen and GetPedInVehicleSeat(veh, 0) ~= 0 then
                Notify('Trap sale cancelled because passenger seat is occupied.', 'error')
                CleanupCustomer(customerPed)
                ResetTrapState()
                return
            end
...
local function StartTrap(mode)
    ...
    if trapActive or saleBusy then
        Notify('Trap sale already active.', 'error')
        return
    end
    ...
    QBCore.Functions.TriggerCallback('moe-drugsale:server:CanStartTrap', function(canStart, reason)
        ...
        if currentMode == 'bulk' then
            Notify('Bulk delivery request sent. Buyer incoming...', 'primary')
        else
            Notify('Trap phone active. Customer incoming...', 'primary')
        end
